...
class IdentifyPancardDocument:
    def __init__(self, text_list: list, logger: object):
        ...
        self.patterns = {
            ...
            'fermanent': re.compile(r'\bfermanent\b', re.IGNORECASE)
        }
    ...
    #     filtered_list = [s for s in self.text_data_list if pattern.search(s)]
    ...
    def find_closest_match(self, target):
        # Use difflib to find the closest match in the text data list
        closest_match = difflib.get_close_matches(target, self.text_data_list, n=1, cutoff=0.1)
        return closest_match[0] if closest_match else None
    ...
    def check_pancard_document_match(self) -> bool:
        # Loop through the target strings and find the closest match
        for target in self.targets:
            match = self.find_closest_match(target)
            if match:
                self.logger.info(f"Found match for Pancard document: {match}")
                return True
        return False